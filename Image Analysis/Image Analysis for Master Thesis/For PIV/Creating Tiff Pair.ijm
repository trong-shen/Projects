  ...
  print(list.length);
  // Iterate through the first folder to create a Tiff Couple folder  
  for (i=0; i<list.length; i++) {
	dir_2=dir+'/'+list[i];
	print(dir_2);
	list_2=getFileList(dir_2);
	if (File.exists(dir_2+"Tiff Couple")!=1 ){
	File.makeDirectory(dir_2+"Tiff Couple");
	}
	     
	//Iterate through each image file to open it
